...
class DocumentValidator {
    ...
    static validateTableTotals(table, warnings) {
        if (!table.totalsRow || table.rows.length === 0)
            return;
        for (let colIdx = 0; colIdx < table.headers.length; colIdx++) {
            ...
            if (typeof totalVal === 'number') {
                ...
                if (numericCount > 0 && Math.abs(sum - totalVal) > 0.01) {
                    warnings.push(`Inconsistência de totais na coluna '${table.headers[colIdx]}': Soma dos dados = ${sum}, Total declarado = ${totalVal}`);
                }
            }
        }
    }
    static sanitizeTableFormulas(table) {
