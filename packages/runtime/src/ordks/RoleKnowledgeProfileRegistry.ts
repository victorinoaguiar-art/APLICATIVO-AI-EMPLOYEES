import { RoleKnowledgeProfile, ExceptionPattern } from '@ai-employee/shared';
import { CANONICAL_500_ROLES } from '@ai-employee/rolepack';

export class RoleKnowledgeProfileRegistry {
  private static instance: RoleKnowledgeProfileRegistry;
  private profilesMap: Map<number, RoleKnowledgeProfile> = new Map();

  private constructor() {
    this.initializeAllProfiles();
  }

  public static getInstance(): RoleKnowledgeProfileRegistry {
  ...
  private initializeAllProfiles(): void {
  ...
  private generateProfileForRole(role: any): RoleKnowledgeProfile {
    ...
    const defaultExceptions: ExceptionPattern[] = [
      {
  ...
  public getRoleProfile(employeeId: number): RoleKnowledgeProfile | undefined {
    return this.profilesMap.get(employeeId);
  }

  public getAllRoleProfiles(): RoleKnowledgeProfile[] {
