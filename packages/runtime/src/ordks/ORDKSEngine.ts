...
import { RoleKnowledgeProfileRegistry } from './RoleKnowledgeProfileRegistry.js';
import { ExceptionLibraryEngine } from './ExceptionLibraryEngine.js';
import { ANGOLA_JURISDICTION_PACK } from './packs/AngolaJurisdictionPack.js';

export class ORDKSEngine {
  private profileRegistry: RoleKnowledgeProfileRegistry;
  private exceptionEngine: ExceptionLibraryEngine;
  private knowledgeBase: Map<string, KnowledgeItem> = new Map();

  constructor() {
    this.profileRegistry = RoleKnowledgeProfileRegistry.getInstance();
    this.exceptionEngine = ExceptionLibraryEngine.getInstance();
    this.seedDefaultKnowledgeItems();
  }

  private seedDefaultKnowledgeItems(): void {
    const items: KnowledgeItem[] = [
      {
        ...
        authorityLevel: KnowledgePrecedenceLevel.PLATFORM_SAFETY_RULES,
        confidence: 1.0,
      ...
      },
      {
        ...
        lastVerifiedAt: '2026-09-10T00:00:00Z',
        verificationStatus: 'ACTIVE',
        authorityLevel: KnowledgePrecedenceLevel.APPLICABLE_LAW_REGULATION,
        confidence: 0.99,
        sensitivity: 'MEDIUM',
  ...
  public queryKnowledge(context: ORDKSQueryContext): ORDKSQueryResult {
    ...
    const invalidStatuses: KnowledgeStatus[] = ['STALE', 'SUPERSEDED', 'DEPRECATED', 'REJECTED', 'QUARANTINED'];

    const matchedItems = Array.from(this.knowledgeBase.values()).filter((item) => {
      if (invalidStatuses.includes(item.verificationStatus)) return false;
    ...
    const angolaHighlights = ANGOLA_JURISDICTION_PACK.legalFrameworks.map(
      (lf) => `[${lf.code}] ${lf.title} (${lf.authority})`
    );
  ...
  public registerKnowledgeItem(item: KnowledgeItem): void {
    this.knowledgeBase.set(item.knowledgeId, item);
  }
}
